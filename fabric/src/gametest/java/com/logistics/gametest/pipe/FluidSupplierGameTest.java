...
public class FluidSupplierGameTest {
    ...
    public void testTargetRoundingRetainsRemainderInBuffer(GameTestHelper context) {
        ...
        context.succeedWhen(() -> {
            ...
            if (tankMb != target) {
                throw context.assertionException("Machine tank should stop at target " + target + " mB, was " + tankMb);
            }
            if (bufferMb != remainder) {
                throw context.assertionException("Buffer should retain " + remainder + " mB, was " + bufferMb);
            }
            if (pendingPackets(context) != 0) {
                throw context.assertionException("Supplier should stop ordering once satisfied");
            }
        });
    }
