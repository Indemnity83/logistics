...
| Fluid logistics | Liquid supplier/provider/request over the network | Modernize | Delivered via a fluid↔item packaging machine so the network stays item-based. The **Transposer** shipped v0.8.6, and the **Fluid Provider** / **Fluid Supplier** pipes and chassis modules shipped with it; liquid **request** is the remaining slice | 🚧 Partial | Phase 1 — fluids |
