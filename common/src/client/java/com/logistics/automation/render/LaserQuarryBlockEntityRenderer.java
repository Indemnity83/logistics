...
public class LaserQuarryBlockEntityRenderer implements BlockEntityRenderer<LaserQuarryBlockEntity, LaserQuarryRenderState> {
    ...
    private void renderFramePreviewOutline(
        ...
        renderOutlineHorizontalEdge(matrices, queue, beamModel, renderLayer, lightmap, relStartX, 0, relEndZ, width, 90);
        renderOutlineHorizontalEdge(matrices, queue, beamModel, renderLayer, lightmap, relStartX, 0, relStartZ, depth, 0);
        ...
        renderOutlineHorizontalEdge(matrices, queue, beamModel, renderLayer, lightmap, relEndX, topY, relStartZ, depth, 0);

        // Vertical corner edges
        renderOutlineVerticalEdge(matrices, queue, beamModel, renderLayer, lightmap, relStartX, 0, relStartZ, height);
        renderOutlineVerticalEdge(matrices, queue, beamModel, renderLayer, lightmap, relEndX, 0, relStartZ, height);
        renderOutlineVerticalEdge(matrices, queue, beamModel, renderLayer, lightmap, relStartX, 0, relEndZ, height);
        renderOutlineVerticalEdge(matrices, queue, beamModel, renderLayer, lightmap, relEndX, 0, relEndZ, height);
    }
    ...
    private void renderOutlineHorizontalEdge(
        ...
        for (int i = 0; i < length; i++) {
            ...
            queue.submitBlockModel(
                    matrices, renderLayer, model, 1f, 1f, 1f, lightmap, OverlayTexture.NO_OVERLAY, 0);
            matrices.popPose();
        }
    }
