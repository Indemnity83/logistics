 ...
 * renderers. MC 26.1 resolves the sprite/tint through the unified vanilla fluid model, but 1.21.x has
 * no such API, so the sprite/tint lookup is delegated to the loader-specific {@link FluidSpriteLookup};
 * the box-drawing geometry below is fully loader-agnostic.
 */
public final class FluidBoxRenderer {
    ...
    /**
     * Render a single flat, double-sided quad on the north (-Z) face at depth {@code z}, spanning
     * {@code [x0,x1] × [y0,y1]}. UV-mapped by block position (tiling, not stretching). Used by the
     * crucible's front-face fluid gauge.
     */
