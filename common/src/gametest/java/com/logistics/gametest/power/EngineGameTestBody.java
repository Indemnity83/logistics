...
/**
 * Shared engine GameTest bodies, compiled directly into both loaders' {@code gametest} source sets
 * (see {@code common/build.gradle}). Loader-specific glue wires these into each loader's own
 * registration mechanism: Fabric's {@code @GameTest}-annotated {@code EngineGameTest} delegates to
 * these methods, and NeoForge's {@code EngineGameTestRegistration} references them directly as
 ...
 */
public class EngineGameTestBody {
    ...
    public static void testStirlingEngineAcceptsFuel(GameTestHelper context) {
        ...
        long inserted = backStorage.insert(ItemStorageLookup.of(new ItemStack(Items.COAL)), 1, false);
        if (inserted != 1) {
    ...
    public static void testStirlingEngineRejectsNonFuel(GameTestHelper context) {
        ...
        long inserted = backStorage.insert(ItemStorageLookup.of(new ItemStack(Items.DIRT)), 1, false);
        if (inserted != 0) {
    ...
    public static void testHopperFillsEmptyEngineFuelSlot(GameTestHelper context) {
        ...
        if (!(context.getBlockEntity(enginePos) instanceof StirlingEngineBlockEntity engine)
                || !(context.getBlockEntity(hopperPos) instanceof HopperBlockEntity hopper)) {
        ...
        context.succeedWhen(() -> context.assertTrue(
                !engine.getTheItem().isEmpty(), "hopper never fed the engine's empty fuel slot"));
    }
}
