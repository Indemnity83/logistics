...
## C. Missing recipe families (predate v0.8.7 but never documented) — ✅ DONE
...
## D. Naming mismatches — ✅ DONE (renamed to match the game)

The real defect was narrower and worse than first described. Icons were *fine* — both icon tools
carried override tables mapping the game's names to the wiki's. What broke was `Module:ItemLink`,
which is generated straight from `en_us.json` and so only knows the game's names: every affected
item fell through to the vanilla branch and linked out to the **Minecraft Wiki** instead of its own
local page.

Pages renamed to the in-game names with redirects left behind (`Fluid Pump`, `ItemSink Module`,
`Mod-Based ItemSink Module`, `Polymorphic ItemSink Module`, `QuickSort Module`); the `Block of X`
and `Chassis … MkN` cases already had correct pages and only needed their grid parameters fixed.
Both override tables are now empty, so the tools are straight mirrors of the lang file again.

Two traps worth remembering: the repo lives on a **case-insensitive filesystem**, so
`Quicksort`/`QuickSort` are the same file — a case-only rename needs a temp hop, and a same-name
redirect stub silently overwrites the page. And a blanket name replacement also rewrites historical
`Grid X (v0.2.0).png` references, so those files have to be renamed alongside.
...
`Fluid Pump` turned out to have a *wrong* harvest tier rather than a missing table — it said
`tier=wood` but the block is in `needs_stone_tool`. Creative-menu tabs are documented on
`Logistics`. `Deepslate Tin Ore` remains covered on `Tin`, and `Creative Sink` remains
undocumented — both creative/variant blocks that do not warrant their own page.
...
## F. Hand-built recipe wikitables — ⛔ WON'T DO (decided)

Both tables stay as wikitables: the `Fuel Engine` one is a properties table rather than a recipe
grid, and the `Reaction Engine` one, while a genuine recipe table, does not justify a whole
template family for three rows.
