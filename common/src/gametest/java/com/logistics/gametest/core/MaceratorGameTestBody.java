...
/**
 * Shared macerator GameTest bodies, compiled directly into both loaders' {@code gametest} source
 * sets (see {@code common/build.gradle}). Loader-specific glue wires these into each loader's own
 * registration mechanism: Fabric's {@code @GameTest}-annotated {@code MaceratorGameTest} delegates
 * to these methods, and NeoForge's {@code MaceratorGameTestRegistration} references them directly
 * as {@code Consumer<GameTestHelper>} method references.
 */
public class MaceratorGameTestBody {
    ...
    public static void testPlacementCreatesBlockEntity(GameTestHelper context) {
        ...
        if ((MaceratorBlockEntity) context.getBlockEntity(pos) == null) {
