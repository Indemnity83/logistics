...
public class FluidPumpBlockEntityRenderer implements BlockEntityRenderer<FluidPumpBlockEntity, FluidPumpRenderState> {
    ...
    public void extractRenderState(
        ...
        if (level != null) {
            ...
            state.tubeLight = LightTexture.pack(blockLight, skyLight);
        }
    }
    ...
    public void submit(
        ...
        // On 1.21.11 submitBlockModel takes a model (not a part list); MachineModels wraps the
        // procedural tube parts in a cached BlockStateModel.
        BlockStateModel model = MachineModels.model("fluid_pump_tube");
        ...
        for (int i = 0; i < tiles; i++) {
            renderSegment(matrices, queue, model, renderLayer, state.tubeLight, tip + i, 1.0f);
        }
    }
    ...
    private void renderSegment(
        ...
        queue.submitBlockModel(matrices, renderLayer, model, 1.0f, 1.0f, 1.0f, light, OverlayTexture.NO_OVERLAY, 0);
        matrices.popPose();
    }
