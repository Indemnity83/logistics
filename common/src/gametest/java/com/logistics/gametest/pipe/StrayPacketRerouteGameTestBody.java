...
public class StrayPacketRerouteGameTestBody {
    ...
    private static PipeContext contextAt(GameTestHelper context, BlockPos pos) {
        return context.getBlockEntity(pos, PipeBlockEntity.class).createContext();
    }
    ...
    private static long chestCount(GameTestHelper context) {
        ChestBlockEntity chest = context.getBlockEntity(CHEST, ChestBlockEntity.class);
    ...
    public static void testStrayItemWithNoOrderStillDrops(GameTestHelper context) {
        ...
        context.succeedWhen(() -> {
            if (!injected[0]) {
                throw context.assertionException("Waiting for the stray stack to be injected");
            }
