 ...
 * for deserts, the red variants for badlands), the {@code result} it turns them into, and the pond's
 * {@code radius} (half-width) and {@code depth} (layers carved down from the surface). The pond only
 * replaces {@code target} blocks, so it naturally stops at the stone under the sand.
 */
