...
public class FluidPumpGameTest {
    ...
    public void testFluidPumpTubeDescendsWithoutEnergy(GameTestHelper context) {
        ...
        context.runAfterDelay(LogisticsConfigHost.get(Configs.FLUID_PUMP_INTERVAL_TICKS) * 2L, () -> {
    ...
    public void testFluidPumpRemovesSourceAndFillsTank(GameTestHelper context) {
        ...
        pump.energyStorage(Direction.NORTH).insert(LogisticsConfigHost.get(Configs.FLUID_PUMP_ENERGY_CAPACITY), false);

        fastArm(pump);

        context.runAfterDelay(LogisticsConfigHost.get(Configs.FLUID_PUMP_INTERVAL_TICKS) + 2, () -> {
    ...
    public void testFluidPumpFindsConnectedSourceInRadius(GameTestHelper context) {
        ...
        context.runAfterDelay(LogisticsConfigHost.get(Configs.FLUID_PUMP_INTERVAL_TICKS) * 4L, () -> {
    ...
    public void testFluidPumpDrainsFinitePool(GameTestHelper context) {
        ...
        context.runAfterDelay(LogisticsConfigHost.get(Configs.FLUID_PUMP_INTERVAL_TICKS) * 6L, () -> {
            for (BlockPos water : pool) {
    ...
    public void testFluidPumpTreatsLargeBodyAsInfinite(GameTestHelper context) {
        ...
        context.runAfterDelay(LogisticsConfigHost.get(Configs.FLUID_PUMP_INTERVAL_TICKS) * 2L + 4, () -> {
            if (pump.tank().getAmount() < FluidUnits.mb(1_000)
                    || pump.tank().getFluidKey().getFluid() != Fluids.WATER) {
    ...
    public void testFluidPumpDrainsConnectedLavaSources(GameTestHelper context) {
        ...
        context.runAfterDelay(LogisticsConfigHost.get(Configs.FLUID_PUMP_INTERVAL_TICKS) * 5L, () -> {
    ...
    public void testFluidPumpDrainsOpenLavaPool(GameTestHelper context) {
        ...
        context.runAfterDelay(LogisticsConfigHost.get(Configs.FLUID_PUMP_INTERVAL_TICKS) * 12L, () -> {
            for (BlockPos p : pool) {
    ...
    public void testFluidPumpStallsAboveSolidFloor(GameTestHelper context) {
        ...
        context.runAfterDelay(LogisticsConfigHost.get(Configs.FLUID_PUMP_INTERVAL_TICKS) * 3L, () -> {
    ...
    private static void fillEnergy(FluidPumpBlockEntity pump) {
        long capacity = LogisticsConfigHost.get(Configs.FLUID_PUMP_ENERGY_CAPACITY);
        long input = Math.max(1, LogisticsConfigHost.get(Configs.FLUID_PUMP_MAX_ENERGY_INPUT));
