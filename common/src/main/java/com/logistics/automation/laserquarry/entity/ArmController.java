...
public final class ArmController {
    ...
    public static long moveCost(long bufferAmount, long moveCostDivisor) {
        return (long) Math
                .ceil(LogisticsConfigHost.get(Configs.QUARRY_ARM_ENERGY) + (double) bufferAmount / moveCostDivisor);
    }
    ...
    public static float effectiveSpeed(long bufferAmount, long moveCostDivisor, Level level, BlockPos quarryPos) {
        ...
        float speed = LogisticsConfigHost.get(Configs.QUARRY_ARM_SPEED) + (mc / LogisticsConfigHost.get(Configs.QUARRY_ARM_SPEED_SCALING));
        if (level != null && level.isRainingAt(quarryPos.above())) {
            speed *= LogisticsConfigHost.get(Configs.QUARRY_RAIN_PENALTY);
        }
        return speed;
    }
