...
/**
 * Shared fluid provider GameTest bodies, compiled directly into both loaders' {@code gametest}
 * source sets (see {@code common/build.gradle}). Loader-specific glue wires these into each loader's
 * own registration mechanism: Fabric's {@code @GameTest}-annotated {@code FluidProviderGameTest}
 * delegates to these methods, and NeoForge's {@code FluidProviderGameTestRegistration} references
 * them directly as {@code Consumer<GameTestHelper>} method references.
 *
 * <p>Tick-based tests for Fluid Provider Pipe packet delivery: max-capped, no-minimum packet sizing,
 * per-physical-packet energy accounting, and no packet ever stacking.
 */
public class FluidProviderGameTestBody {
    ...
    public static void testFluidProviderMintsAndDeliversPackets(GameTestHelper context) {
        ...
        context.runAfterDelay(15, () -> {
            ...
            context.succeedWhen(() -> {
                ...
                context.assertTrue(remainingMb == fillMb - maxMb * 4,
    ...
    public static void testFluidProviderDispatchesBelowOnePacketSize(GameTestHelper context) {
        ...
        context.succeedWhen(() -> {
            GlassTankBlockEntity t = (GlassTankBlockEntity) context.getBlockEntity(tankPos);
            long remainingMb = FluidUnits.toMillibuckets(t.amount());
            context.assertTrue(remainingMb == 0, "Sub-max tank should fully drain, remaining mB: " + remainingMb);

            ChestBlockEntity chest = (ChestBlockEntity) context.getBlockEntity(chestPos);
            int packetCount = packetCount(chest);
            context.assertTrue(packetCount == 1, "Exactly one packet should be dispatched, found " + packetCount);
            java.util.List<ItemStack> stacks = packetStacks(chest);
            FluidPacket data = stacks.get(0).get(LogisticsPipe.DATA.FLUID_PACKET);
            context.assertTrue(data != null && data.amountMb() == fillMb, "Dispatched packet should carry the full remainder ("
                    + fillMb + " mB), was " + (data == null ? "null" : data.amountMb()));

            long energyNow = junction.energyStorage(null).getAmount();
            context.assertTrue(energyNow == startEnergy - rf, "Exactly one packet's worth of energy should be charged");
        });
    }
    ...
    public static void testMixedFullPlusTailDispatchChargesPerPhysicalPacket(GameTestHelper context) {
        ...
        context.runAfterDelay(15, () -> {
            ...
            context.succeedWhen(() -> {
                ChestBlockEntity chest = (ChestBlockEntity) context.getBlockEntity(chestPos);
                java.util.List<ItemStack> stacks = packetStacks(chest);
                context.assertTrue(stacks.size() == 3, "Expected exactly 3 physical packets, found " + stacks.size());
                ...
                for (ItemStack stack : stacks) {
                    context.assertTrue(stack.getCount() == 1,
                            "Every packet must be count=1 (packets never stack), found " + stack.getCount());
                    FluidPacket data = stack.get(LogisticsPipe.DATA.FLUID_PACKET);
                    context.assertTrue(data != null, "Packet stack missing its FluidPacket component");
                    amounts.add(data.amountMb());
                }
                java.util.Collections.sort(amounts);
                context.assertTrue(amounts.equals(java.util.List.of(tailMb, maxMb, maxMb)),
                        "Expected packets of " + maxMb + " mB (x2) and " + tailMb + " mB (x1), found " + amounts);

                long energyNow = junction.energyStorage(null).getAmount();
                context.assertTrue(energyNow == startEnergy - 3 * rf,
