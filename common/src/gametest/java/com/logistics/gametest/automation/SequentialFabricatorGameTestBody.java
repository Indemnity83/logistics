...
/**
 * Shared Sequential Fabricator GameTest bodies, compiled directly into both loaders' {@code
 * gametest} source sets (see {@code common/build.gradle}). Loader-specific glue wires these into
 * each loader's own registration mechanism: Fabric's {@code @GameTest}-annotated
 * {@code SequentialFabricatorGameTest} delegates to these methods, and NeoForge's
 * {@code SequentialFabricatorGameTestRegistration} references them directly as
 * {@code Consumer<GameTestHelper>} method references.
 */
public class SequentialFabricatorGameTestBody {
