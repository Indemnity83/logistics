...
## B. v0.8.7 content that is missing entirely
...
* `Sturdy Casing` and `Tin Gear` are written as removed items but still sit in
  `[[Category:Materials]]`; every other removed item is in `[[Category:Removed]]` only. Missed by
  commit `04fd07c2`.
