...
public final class QuarryBlockBreaker {
    ...
    public static void mineBlock(ServerLevel world, BlockPos target, BlockState targetState, QuarryOutput output) {
        ...
        if (blockEntity != null) {
            ...
            // A spill can land in a chunk section no entity query can see yet, so one look is not
            // enough -- keep sweeping this spot for a moment (see QuarryOutput#sweepAgainFor).
            output.sweepAgainFor(target, alreadyOnGround);
        }
    }
}
