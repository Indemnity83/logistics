...
/**
 * Gametests for the Transposer: bucket ⇄ fluid conversion and, most importantly, atomicity — a blocked
 * output slot must leave both the input item and the tank untouched.
 */
public class TransposerGameTest {
    ...
    private static TransposerBlockEntity place(GameTestHelper context, BlockPos pos) {
        ...
        TransposerBlockEntity be = (TransposerBlockEntity) context.getBlockEntity(pos);
        if (be == null) {
    ...
    private static Fluid liquidRedstone() {
        return BuiltInRegistries.FLUID.get(LogisticsCore.resource("liquid_redstone").toIdentifier());
    }
    ...
    /** Empty bucket + tank of lava → lava bucket out, tank drained by 1000 mB. */
    ...
    /** Empty bucket + tank of a custom mod fluid → that fluid's bucket out, tank drained by 1000 mB. */
    @GameTest(template = "fabric-gametest-api-v1:empty", timeoutTicks = 40)
    ...
    /** Filled bucket of one fluid against a tank holding another → rejected: input and tank untouched. */
    @GameTest(template = "fabric-gametest-api-v1:empty", timeoutTicks = 40)
